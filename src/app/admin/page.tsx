...
import { db, analyticsEvent, authUser, waitlistSignup } from '@/lib/db';
...
const CARD = 'border-border bg-surface rounded-lg border';
...
async function loadStats() {
  const [byEvent, methods, signups, recent, userTotals, waitlist] = await Promise.all([
    db
      .select({
    ...
    db
      .select({ email: waitlistSignup.email, createdAt: waitlistSignup.createdAt })
      .from(waitlistSignup)
  ...
  return (
    <main className="mx-auto w-full max-w-5xl px-6 py-12">
      ...
      <section className="mb-10 grid gap-4 sm:grid-cols-3">
        ...
        <Stat label="Waitlist" value={s.waitlist.length} />
