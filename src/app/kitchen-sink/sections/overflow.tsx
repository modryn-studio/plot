    ...
    >
      <Row label="Long title in a card" note="wraps and balances, never clips">
        <div className="border-border max-w-sm rounded-md border p-6">
          <h4 className="text-title3 text-balance">{LONG_TITLE}</h4>
          <p className="text-body1 text-muted mt-2 text-pretty">
      ...
      <Row label="Unbroken token" note="no spaces, so nothing can wrap it without help">
        <div className="border-border max-w-sm rounded-md border p-6">
          <p className="text-body1 break-all">{LONG_TOKEN}</p>
          ...
          {ROWS.map((row) => (
            <div key={row.email} className="flex items-center gap-4 p-4">
              <div className="min-w-0 flex-1">
                <p className="text-body1 truncate">{row.name}</p>
                <p className="text-subtitle4 text-muted truncate">{row.email}</p>
              </div>
              <span className="text-subtitle4 text-muted shrink-0">Active</span>
            </div>
          ))}
        </div>
        <p className="text-subtitle4 text-muted mt-2">
        ...
        <div className="border-border divide-rule max-h-96 max-w-md divide-y overflow-y-auto rounded-md border">
          {Array.from({ length: 60 }, (_, i) => (
            <div key={i} className="flex items-center justify-between gap-4 p-3">
              <span className="text-body1 tabular-nums">Item {String(i + 1).padStart(3, '0')}</span>
              <span className="text-subtitle4 text-muted tabular-nums">{(i * 137) % 1000}</span>
            </div>
          ))}
        </div>
        <p className="text-subtitle4 text-muted mt-2">
