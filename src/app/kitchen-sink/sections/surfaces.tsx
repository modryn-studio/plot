    ...
    >
      ...
      <Row label="Border and rule" note="two weights, two jobs: a control edge is found, a divider only separates">
        {/* `bg-elevated` ON THE GROUP, AND THAT IS THE RULE RATHER THAN THE DEMO'S PREFERENCE
            ...
            `rule` is "measured relative to `elevated`". Drawn on the page it is out of spec, and a
            project whose `rule` happens to equal its `bg` gets zero contrast rather than a faint
            line: run-rebuild's do, and its dividers vanished here entirely. Reviewing a token on the
            one ground it is never used on is how that survives. */}
